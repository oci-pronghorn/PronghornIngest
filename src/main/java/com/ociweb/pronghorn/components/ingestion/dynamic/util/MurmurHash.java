...
public class MurmurHash {
    ...
    @SuppressWarnings("fallthrough")
    public static int hash32(byte[] src, int srcOffset, int srcLength, int srcMask, int seed) {
        // Initialize the hash to a 'random' value
        int h = seed ^ srcLength;

        int i = srcOffset;
        int len = srcLength;
        while (len >= 4) {
            int k = src[srcMask & i] & 0xFF;
            k |= (src[srcMask&(i + 1)] & 0xFF) << 8;
            k |= (src[srcMask&(i + 2)] & 0xFF) << 16;
            k |= (src[srcMask&(i + 3)] & 0xFF) << 24;
        ...
        switch (len) {
        case 3:
            h ^= (src[srcMask&(i + 2)] & 0xFF) << 16;
        case 2:
            h ^= (src[srcMask&(i + 1)] & 0xFF) << 8;
        case 1:
            h ^= (src[srcMask&i] & 0xFF);
            h *= MURMUR2_MAGIC;
        }
